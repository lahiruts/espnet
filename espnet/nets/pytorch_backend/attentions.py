...
class AttFactorizedLoc(torch.nn.Module):
    """location-aware attention
    ...
    def __init__(self, eprojs, dunits, att_dim, aconv_chans, aconv_filts, gatt_dim, att_scale, gatt_scale):
        ...
        self.global_attention_mlp_1 = torch.nn.Linear(eprojs, gatt_dim, bias=False)
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev, scaling=2.0):
        ...
        if self.global_attention is None:
            ...
            self.global_attention = self.g_adapt + torch.sum(self.enc_h * self.global_w.view(batch, self.h_length, 1), dim=1)
