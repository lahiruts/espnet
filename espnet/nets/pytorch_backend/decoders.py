...
class MultiLayerGlobalAttentionDecoder(torch.nn.Module):
    ...
    def forward(self, hs_pad1, hs_pad2, hlens, ys_pad, strm_idx=0):
        """Decoder forward
        ...
        # initialization
        c_list = [self.zero_state(hs_pad1)]
        z_list = [self.zero_state(hs_pad1)]
        for _ in six.moves.range(1, self.dlayers):
            c_list.append(self.zero_state(hs_pad1))
            z_list.append(self.zero_state(hs_pad1))
        att_w = None
        z_all = []
        self.att[att_idx].reset()  # reset pre-computation of h
    ...
    def recognize_beam(self, h, h1, lpz, recog_args, char_list, rnnlm=None, strm_idx=0):
        """beam search implementation
    ...
    def calculate_all_attentions(self, hs_pad, hs_pad1, hlen, ys_pad, strm_idx=0):
        """Calculate all of attentions
    ...
    def calculate_all_utterance_attentions(self, hs_pad, hs_pad1, hlen, ys_pad, strm_idx=0):
        """Calculate all of attentions
