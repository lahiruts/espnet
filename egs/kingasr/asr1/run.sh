...
# data
hkust1=/mnt/nas/HKUST_corpus/hkust_mcts_p1/
hkust2=/mnt/nas/HKUST_corpus/hkust_mcts_p1tr/
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 41 --write_utt2num_frames true \
        data/train_tr90 exp/make_fbank/train_tr90 ${fbankdir}
    utils/fix_data_dir.sh data/train_tr90
    steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 4 --write_utt2num_frames true \
        data/train_cv10 exp/make_fbank/train_cv10 ${fbankdir}
    utils/fix_data_dir.sh data/train_cv10

    steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 5 --write_utt2num_frames true \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    mkdir -p ${lmdatadir}
    text2token.py -s 1 -n 1 -l ${nlsyms} data/train_tr90/text | cut -f 2- -d" " \
        > ${lmdatadir}/train.txt
